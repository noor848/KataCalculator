...
namespace Kata_Calculator
{
    public class Precedence
    {
        public static void preceding(string upc)
        {
            ...
            double FinalProduct = product.RemainingPrice + product.TaxAmount - product.GetUniversalDiscountAmount(product.RemainingPrice);
            ...
            Console.WriteLine($"Program Reports Total Discount Amount For " +
                $"UPC = {upc} Is { Report.TwoDecimalPlaces(product.GetUniversalDiscountAmount(product.RemainingPrice)+ product.UPCDiscount(product.ProductPrice))}");
